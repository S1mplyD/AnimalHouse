function getNews() {
    $.getJSON('/api/news', function (jd) {
        ...
        mainZone.append('<label for="picture">Picture:</label><br><input type="file" value="Choose your photos" name="picture" id="files" /><br>');
        mainZone.append('<input id="submit_add" type="button" value="SUBMIT!" onclick="createNews()" /></form>');
        mainZone.append('<br>');
        mainZone.append('<form id="new_serv"><label for="title1">ID of the news to remove</label><br><input type="text" id="title1" name="title1"><br>');
        mainZone.append('<button id="submit_delete" type="button" onclick="deleteNews()">DELETE!</button></form>');
