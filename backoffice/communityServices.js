async function getPosts() {
  ...
  if (login.data != "") {
    $.getJSON("/api/posts", function (jd) {
      ...
      mainZone.append(
        '<label for="picture">Picture:</label><br><input type="file" value="Choose your photos" name="picture" id="files" multiple /><br>'
      );
      mainZone.append(
        '<input id="submit_add" type="button" value="SUBMIT!" onclick="createPost()" /></form>'
      );
      mainZone.append("<br>");
      mainZone.append(
        '<form id="new_serv"><label for="title1">ID of the post to remove</label><br><input type="text" id="title1" name="title1" placeholder="id"><br>'
      );
      mainZone.append(
        '<button id="submit_delete" type="button" onclick="deletePost()">DELETE!</button></form>'
      );
    });
  }
}
