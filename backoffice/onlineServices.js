async function getOnlineServices() {
  ...
  if (login.data != "") {
    $.getJSON("/api/services", function (jd) {
      ...
      mainZone.html(
        '<table id="table"><tr><th>Name</th><th>Id</th><th>Location</th><th>Time of opening/th><th>Type</th><th>Info</th></tr>'
      );
      ...
      mainZone.append(
        '<button id="submit_buton" type="button" onclick="createOnlineService()">SUBMIT</button></form>'
      );
      mainZone.append(
        '<form id="new_serv"><label for="title">ID of the service to delete:</label><br><input type="text" id="title" name="title" placeholder="id">'
      );
      mainZone.append(
        '<button id="submit_remon" type="button" onclick="deleteOnlineService()">REMOVE SERVICE</button></form>'
      );
      mainZone.append(
      ...
      mainZone.append(
        '<button id="update_buton" type="button" onclick="updateOnlineService()">UPDATE</button></form>'
      );
    });
  }
}
